...
Restop is a small, read-only web browser for one [restic](https://restic.net/) repository. It lists snapshots, browses and searches their directory trees, and streams files or directories for download. Directory downloads are uncompressed tar archives produced directly by `restic dump`.
...
Restop intentionally has no mutation endpoints, database, metadata cache, preview, or multi-repository mode. Search is scoped to one snapshot at a time. The embedded HTMX asset enhances navigation, but every browsing link, search, and download continues to work with JavaScript disabled.
